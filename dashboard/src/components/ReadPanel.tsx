...
function HolderCard({
  ...
  const legacy = isLegacyContract(h.holder);

  return (
    <div className={`card-block holder-card${legacy ? " legacy" : ""}`}>
...
function TokenModals({
  ...
  verifyAddr: Address | null;
  removeToken: { symbol: string; address: Address } | null;
  setVerifyAddr: (a: Address | null) => void;
  setRemoveToken: (t: { symbol: string; address: Address } | null) => void;
  onRefresh: () => void;
}) {
  return (
    <>
...
export function BalancesPanel({ data, loading, onRefresh }: ReadProps) {
  ...
  return (
    <section className="panel">
      ...
      <p className="section-note">
        SCRATCH <span className="mono">{fmtUsd(data?.prices.scratchUsd ?? null)}</span>
        {" · "}
...
export function StakingOpsPanel({ data, loading, onRefresh }: ReadProps) {
  return (
    <section className="panel">
      <div className="panel-head">
        <h2>Staking &amp; ops</h2>
      ...
      <p className="section-note">
        Vitals for ticket sources, vesting, and the active game. Staker enumeration lives under
        Stakers (v2 snapshot).
      </p>
...
/** @deprecated Prefer BalancesPanel / StakingOpsPanel — kept for any external imports. */
export function ReadPanel(props: ReadProps) {
  return (
    <>
      <BalancesPanel {...props} />
      <StakingOpsPanel {...props} />
    </>
  );
}
